...
export class CarbonAccountingRouter {
    ...
    private async recordAuditedEmissionToken(req: Request, res: Response) {
        ...
        for (const uuid of emissionsRecordsToAudit) {
            ...
            try {
                emission = await this.opt.utilityEmissionChannel.getEmissionsData(userId, orgName, {
                    uuid,
                });
        ...
        try {
            await this.opt.utilityEmissionChannel.updateEmissionsMintedToken(userId, orgName, {
                tokenId,
                partyId,
                uuids: manifestIds,
            });
