...
export class UtilityEmissionsChannelRouter {
    ...
    private async recordEmissions(req: Request, res: Response) {
        ...
        this.log.debug(
            `${fnTag} fetching allEmissionRecords with utilityId=${utilityId} , partyId=${partyId}`,
        );
        try {
            emissionRecords = await this.opts.utilityEmissionsChannel.getAllEmissionRecords(
                userId,
                orgName,
                { utilityId, partyId },
            );
        ...
        for (const emission of emissionRecords) {
            ...
            if (overlap) {
                this.log.info(
                    `${fnTag} Supplied dates ${fromDate} to ${thruDate} overlap with an existing dates ${emission.fromDate} to ${emission.thruDate}`,
                );
    ...
    private async getEmissionsData(req: Request, res: Response) {
        ...
        try {
            const emissionRecord = await this.opts.utilityEmissionsChannel.getEmissionsData(
                userId,
                orgName,
                { uuid },
            );
    ...
    private async getAllEmissionData(req: Request, res: Response) {
        ...
        try {
            const result = await this.opts.utilityEmissionsChannel.getAllEmissionRecords(
                userId,
                orgName,
                { utilityId, partyId },
            );
    ...
    private async getAllEmissionsDataByDateRange(req: Request, res: Response) {
        ...
        try {
            const result = await this.opts.utilityEmissionsChannel.getAllEmissionsDataByDateRange(
                userId,
                orgName,
                { fromDate, thruDate },
            );
