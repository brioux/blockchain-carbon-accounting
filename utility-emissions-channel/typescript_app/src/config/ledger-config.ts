...
export function getLedgerConfigs(): ILedgerIntegrationConfig {
    ...
    if (ethNetwork === 'hardhat') {
        ...
        throw new Error(
            'LEDGER_ETH_NETWORK : xdai || goerli || ropsten ethereum network are supported',
        );
    }
...
export function readUtilityEmissionChaincodeCfg(cfgPath: string): IFabricChaincodeConfig {
    ...
    const certificateAuthorities: { [key: string]: any } = {};
    const peers: { [key: string]: any } = {};
    const orderers: { [key: string]: any } = {};
    const organizations: { [key: string]: any } = {};
