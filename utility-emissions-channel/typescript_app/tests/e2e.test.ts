...
describe('E2E', () => {
    ...
    it('getEmissions should pass', (done) => {
        chai.request(v1Base)
            .get(apiEndpoint['getEmissionsData'] + `${testUserId}/auditor1/${emissionsId}`)
            .set('content-type', 'application/x-www-form-urlencoded')
            .end((err, response) => {
                try {
                    ...
                    assert.equal(response.body.factorSource, 'eGrid 2019 NERC_REGION MRO');
                    done();
                } catch (error) {
                    done(err);
                }
...
async function insertMockData(registry: PluginRegistry) {
    const config = readUtilityEmissionChaincodeCfg(join(__dirname, '..', 'config.json'));
